...
extern uthread_struct_t *sched_find_best_uthread(kthread_runqueue_t *kthread_runq)
{
	...
	kthread_context_t *current_k_ctx, *other_k_ctx;
	current_k_ctx = kthread_cpu_map[kthread_apic_id()];
	/********************************************/
	...
	{ /* No jobs in active. switch runqueue */
		...
		/**************search for active in other kthreads*********************/

		if(policy==1)
		{
			//fprintf(stderr, "IN LOAD BALANCER########################################\n");
			gt_spin_unlock(&(kthread_runq->kthread_runqlock));
		
			for(i = 0; i!=current_k_ctx->cpuid && i<NUM_CPUS; i++)
			{
				other_k_ctx = kthread_cpu_map[i];
				other_kthread_runq = &(other_k_ctx->krunqueue);
				...
				else
				{
					...
					gt_spin_unlock(&(other_kthread_runq->kthread_runqlock));
#if 1
					printf("Thread: %d, group: %d removed from CPU:%d\n", u_obj->uthread_tid, u_obj->uthread_gid, u_obj->cpu_id);
#endif
					return(u_obj);
				}
			}
