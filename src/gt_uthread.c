...
extern void gt_yield()
{
	...
	kthread_runq=&(k_ctx->krunqueue);
    	//printf("Entered the gt_yield()!!!\n");
    	if(u_obj=kthread_runq->cur_uthread); //if cpu is running the preempted thread, set the flag to 1 and find new uthread to run on the cpu.
    	{
    		u_obj->gt_yield_flag=1;
    		printf("Thread id= %d yielded",u_obj->uthread_tid);
    	}
...
extern void uthread_schedule(uthread_struct_t * (*kthread_best_sched_uthread)(kthread_runqueue_t *))
{
	...
	k_ctx = kthread_cpu_map[kthread_apic_id()]; //use this fr stealig in pq.c
	kthread_runq = &(k_ctx->krunqueue);
	...
	if((u_obj = kthread_runq->cur_uthread))
	{
		if (policy==1)
		{
			gettimeofday(&u_obj->time_prempted, NULL); // to get the premepted time
			// the actual time spent executing= prempted time-scheduled time
			timersub(&u_obj->time_prempted,&u_obj->time_scheduled,&u_obj->run_time_temp);
            // adding this time to get the total run time for the given thread
            timeradd(&u_obj->run_time,&u_obj->run_time_temp,&u_obj->run_time);
            // deducting credits on the basis of time
            u_obj->cycles_used = ((float)(u_obj->run_time_temp.tv_sec*1000000 + u_obj->run_time_temp.tv_usec))/(KTHREAD_VTALRM_SEC*1000000 + KTHREAD_VTALRM_USEC);
            u_obj->credit-=(int)(u_obj->cycles_used*25);
		}
		//printf("policy variable: %d", policy);
		...
		if(u_obj->uthread_state & (UTHREAD_DONE | UTHREAD_CANCELLED))
		{
			if(policy==1)
			{
				...
				uthread_time[u_obj->uthread_tid]->id= u_obj->uthread_tid;
				timersub(&u_obj->endtime,&u_obj->starttime,&uthread_time[u_obj->uthread_tid]->totaltime);//to get the total time= end time-start time
				uthread_time[u_obj->uthread_tid]->runtime=u_obj->run_time;
				fprintf(stderr, "Thread id: %u group id: %u completed its execution with total time= %lu s and %lu us and run time =%lu s and %lu us\n", u_obj->uthread_tid, u_obj->uthread_gid, uthread_time[u_obj->uthread_tid]->totaltime.tv_sec, uthread_time[u_obj->uthread_tid]->totaltime.tv_usec, uthread_time[u_obj->uthread_tid]->runtime.tv_sec, uthread_time[u_obj->uthread_tid]->runtime.tv_usec);
		...
		else
		{
			...
			//printf("Let's check if gt_yield would work\n");
			if (policy!=1)
			{
			...
			{
				//printf("credit scheduler section\n");
				if (u_obj->credit >0)
					{
						if(u_obj->gt_yield_flag==1)
						{
							u_obj->credit+=5;
							//printf("the yield part is completed");
							add_to_runqueue(kthread_runq->expires_runq, &(kthread_runq->kthread_runqlock), u_obj);
							u_obj->gt_yield_flag=0;
						}
						else
						{
							u_obj->credit-=25;
				...
				{
					u_obj->credit=u_obj->init_credit;
					u_obj->gt_yield_flag=0;
					printf("The thread with id: %d is in expired runqueue with replinshed credits= %d\n", u_obj->uthread_tid, u_obj->credit);
					add_to_runqueue(kthread_runq->expires_runq, &(kthread_runq->kthread_runqlock), u_obj);
	...
	gettimeofday(&u_obj->time_scheduled, NULL);//this is when the thread is added in the queue measuring the scheduled time.
...
static void uthread_context_func(int signo)
{
	...
	gettimeofday(&cur_uthread->endtime, NULL); //end time calculation when the uthread is done with the execution
...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, uthread_group_t u_gid, int weight)
{
	...
	// u_new->init_credit=weight;  have to change to this
	u_new->init_credit=weight; 
	u_new->credit= u_new->init_credit; 
	gettimeofday(&u_new->starttime, NULL);
	u_new->gt_yield_flag=0;
	...
	printf("the thread with thread id: %d created in uthread.c\n",u_new->uthread_tid);

	/* XXX: ksched_find_target should be a function pointer */
	kthread_runq = ksched_find_target(u_new);

	*u_tid = u_new->uthread_tid;
	uthread_time[u_new->uthread_tid]=(uthread_time_t*)MALLOCZ_SAFE(sizeof(uthread_time_t));
...
int main()
{
	...
	for(inx=0; inx<NUM_THREADS; inx++)
	{
		...
		uarg->num3 = 0x55;
		uarg->num4 = 0x77;
		uthread_create(&u_tid, func, uarg, (inx % MAX_UTHREAD_GROUPS), weight);
	}
